...
ID3D11Buffer* g_pVBuffer = nullptr;     // 정점 데이터를 담는 바구니
...
float g_posX = 0.0f; // 육망성의 X 좌표 (초기값 0.0f)
float g_posY = 0.0f; // 육망성의 Y 좌표 (초기값 0.0f)
float g_speed = 0.0005f; // 육망성의 이동 속도 (초당 0.01f)

Vertex g_vertices[6]; // 육망성의 정점 데이터 (6개의 삼각형으로 구성)

// 현재 posX, posY를 기준으로 6개의 정점 좌표를 새로 찍는 함수
void UpdateStar() {
  // --- 삼각형 1
  g_vertices[0] = { g_posX + 0.0f, g_posY + 0.5f, 0.5f, 1.0f, 0.0f, 0.0f, 1.0f };
  g_vertices[1] = { g_posX + 0.5f, g_posY - 0.5f, 0.5f, 0.0f, 1.0f, 0.0f, 1.0f };
  g_vertices[2] = { g_posX - 0.5f, g_posY - 0.5f, 0.5f, 0.0f, 0.0f, 1.0f, 1.0f };

  // --- 삼각형 2
  g_vertices[3] = { g_posX + 0.0f, g_posY - 0.83f, 0.5f, 1.0f, 0.0f, 0.0f, 1.0f };
  g_vertices[4] = { g_posX - 0.5f, g_posY + 0.17f, 0.5f, 0.0f, 1.0f, 0.0f, 1.0f };
  g_vertices[5] = { g_posX + 0.5f, g_posY + 0.17f, 0.5f, 0.0f, 0.0f, 1.0f, 1.0f };
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
  ...
  while (WM_QUIT != msg.message) {
    ...
    if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
      ...
      // (1) 업데이트 단계: 방향키 입력 처리
      if (GetAsyncKeyState(VK_LEFT))  g_posX -= g_speed;
      if (GetAsyncKeyState(VK_RIGHT)) g_posX += g_speed;
      if (GetAsyncKeyState(VK_UP))    g_posY += g_speed;
      if (GetAsyncKeyState(VK_DOWN))  g_posY -= g_speed;
